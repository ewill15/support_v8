...
@section('title', ucfirst(trans('common.type_users')))
@section('content')
    <!-- ============================================================== -->
    <!-- pageheader -->
    <!-- ============================================================== -->
            ...
            <div class="row">
                <div class="col-xl-8 col-lg-8 col-md-12 col-sm-12 col-12">
                    <div class="page-header">
                        <h2 class="pageheader-title">{{ ucfirst(trans('common.type_users')) }}</h2>
                        <div class="page-breadcrumb">
                            <nav aria-label="breadcrumb">
                                <ol class="breadcrumb">
                                    <li class="breadcrumb-item"><a href="{{ url('admin/dashboard') }}" class="breadcrumb-link">{{ ucfirst(trans('common.home')) }}</a></li>
                                    <li class="breadcrumb-item active">{{ ucfirst(trans('common.list_type_users')) }}</li>
                                </ol>
                            </nav>
                        </div>
                    </div>
                ...
                <div class="col-xl-4 col-lg-4 col-md-12 col-sm-12 col-12">
                    @if(@$new)
                        <div class="card-body border-top">
                            <a href="{{ url('admin/type_users/create') }}" class="btn btn-outline-primary float-right">
        ...
        <div class="container-fluid">
            ...
            <div class="row">
                ...
                <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
                    <div class="card">
                        <h5 class="card-header">{{ucfirst(trans('common.type_users'))}}</h5>
                        <div class="card-body">
                            <div class="ibox-content">
                                <div class="table-responsive">
                                    <form  id="paginate" action="{{ url('admin/type_users') }}">
                                        <div class="row">
                                    ...
                                    <table class="table table-striped table-bordered table-hover">
                                        ...
                                        <tr>
                                            <th>ID</th>
                                            <th>{{ ucfirst(trans('common.type_user')) }}</th>
                                        ...
                                        @foreach($type_users as $item)
                                            <tr class="gradeX {{$item->deleted_at?'text-danger':''}}">
                                                ...
                                                <td>
                                                    @if ($item->deleted_at)
                                                        @if(@$recycle)
                                                            <a class="btn btn-info btn-sm btn-restore" href="#"
                                                                ...
                                                                data-action="restore"
                                                                data-url="{{ asset('admin/type_users/restore/'.@$item->id)}}"
                                                                data-title-msg="{{ ucfirst(trans('common.restore_type_user')) }}"
                                                    ...
                                                    @else
                                                        @if(@$edit)
                                                            <a href="{{ url('admin/type_users/' . @$item->id . '/edit') }}" class="btn btn-primary btn-sm" data-toggle-tip="tooltip" data-placement="top" title="{{ ucfirst(trans('common.update')) }}">
                                                                <i class="fas fa-edit"></i>
                                                            </a> 
                                                        @endif
                                                        @if(@$recycle)
                                                            <a class="btn btn-warning btn-sm btn-recycle" href="#"
                                                                data-toggle-tip="tooltip" data-placement="top" 
                                                                title="{{ ucfirst(trans('common.recycle')) }}"
                                                                data-action="recycle"                                                    
                                                                data-url='{{ asset('admin/type_users/soft/'.@$item->id)}}'
                                                                data-name="{{ $item->name_type_user }}"
                                                        ...
                                                        @if(@$trash)
                                                            <a href="#" class="btn btn-danger btn-sm" 
                                                                ...
                                                                data-action="delete"
                                                                data-name=" {{ $item->name_type_user }}"
                                                                data-url="{{ route('type_users.destroy', $item->id) }}"
                                                                data-title-msg="{{ ucfirst(trans('common.msgdelete_register')) }}"
                                                                data-text-msg="{{ ucfirst(trans('common.msgdelete')) }}"
                                    ...
                                    @include('admin.type_users.partials.modal-delete')
                                    @include('admin.type_users.partials.modal-recycle')
                                    @include('admin.type_users.partials.modal-restore')
                                    <div class="float-left">
                                        {{ $text_pagination }}
                                    </div>
                                    <div class="float-right">                                        
                                        <div class="btn-group">
                                          {!! $type_users->appends(request()->except('page'))->render() !!}
                                        </div>
                                    </div>
                                </div>
